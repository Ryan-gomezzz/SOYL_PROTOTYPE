...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  ...
  if (event.httpMethod === 'GET') {
    return { statusCode: 200, body: JSON.stringify({ msg: 'designs lambda healthy' }) };
  }
  ...
  if (!brief) return { statusCode: 400, body: JSON.stringify({ error: 'brief required' }) };
...
Do NOT include explanatory text outside JSON. If you cannot produce a design, return {"error":"explain reason"}.
  ...
  while (attempt < 2) {
    ...
    try {
      ...
      if (attempt >= 2) {
        return { statusCode: 502, body: JSON.stringify({ error: 'LLM provider error' }) };
      } else {
